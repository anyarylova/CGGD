...
void cg::renderer::ray_tracing_renderer::render()
{
	...
	//Adjusted for Monte-Carlo light tracing
	raytracer->closest_hit_shader = [&](const ray& ray, payload& payload, const triangle<cg::vertex> triangle, size_t depth) {
		...
		float u = uniform_dist(random_generator);
		float v = uniform_dist(random_generator);
		float theta = 2.f * 3.14159265358979323846f * u;
		float phi = acos(2.f * v - 1.f);
		float x = cos(theta) * sin(phi);
		float y = sin(theta) * sin(phi);
		float z = cos(phi);
		float3 tangent = (std::fabs(normal.x) > std::fabs(normal.y))
				? normalize(float3(-normal.z, 0, normal.x))
				: normalize(float3(0, -normal.z, normal.y));
		float3 bitangent = cross(normal, tangent);

		float3 random_direction = normalize(x * tangent + y * bitangent + z * normal);
	...
	shadow_raytracer->any_hit_shader = [](const ray&, payload& payload, const triangle<cg::vertex>& triangle) {
		return payload;
	};
