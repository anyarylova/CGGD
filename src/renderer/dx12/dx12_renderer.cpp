...
void cg::renderer::dx12_renderer::create_root_signature(const D3D12_STATIC_SAMPLER_DESC* sampler_descriptors, UINT num_sampler_descriptors)
{
	...
	CD3DX12_VERSIONED_ROOT_SIGNATURE_DESC rs_desk;
	rs_desk.Init_1_1(_countof(root_parameters), root_parameters, num_sampler_descriptors, sampler_descriptors, rs_flags);
	ComPtr<ID3DBlob> signature;
	ComPtr<ID3DBlob> error;
	HRESULT res = D3DX12SerializeVersionedRootSignature(&rs_desk, rs_feature_data.HighestVersion, &signature, &error);
	if (FAILED(res)) {
...
#endif
	HRESULT res = D3DCompileFromFile(shader_path.wstring().c_str(), nullptr, nullptr, entrypoint.c_str(), target.c_str(), compile_flags, 0, &shader, &error);
	if (FAILED(res)) {
		OutputDebugStringA((char*)error->GetBufferPointer());
		THROW_IF_FAILED(res);
	}
